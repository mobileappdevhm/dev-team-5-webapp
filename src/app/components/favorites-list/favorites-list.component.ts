...
export class FavoritesListComponent implements OnInit {
  ...
  rmFromFav(param: Favorites) {
    ...
    this.http.delete(SERVER_URL + '/courses/' + param.id)
      .subscribe(res => {
        console.log(res);
        this.getFavorites();
      },
  ...
  checkTimeConflict() {
    ...
    if (this.x == 0) {
      for (const entry of this.favList) {
        if (entry.CourseLocation == 'Lothstrasse') {
          const myDate = new tempDate(entry.CourseLocation, entry.CourseName,
    ...
    //console.log(this.favLoth);
      ...
      for (let i = 0; i < this.favLoth.length - 1; i++) {
        for (let j = i + 1; j < this.favLoth.length; j++) {
          if (this.favLoth[i].start.getDay() === this.favLoth[j].start.getDay()) {

            const tempEndCurrent = this.favLoth[i].end.getHours() * 60 + this.favLoth[i].end.getMinutes();
            const tempStartNext = this.favLoth[j].start.getHours() * 60 + this.favLoth[j].start.getMinutes();

            if ((tempStartNext - tempEndCurrent) < 45) {
              const hint = 'conflict found between ' + this.favLoth[i].name + ' and ' + this.favLoth[j].name + '!';
              this.report.push(hint);
            } else {
              const hint = 'no conflict between ' + this.favLoth[i].name + ' and ' + this.favLoth[j].name;
              this.report.push(hint);
  ...
  parseDate(param: String): Date {
    ...
    const month = param.substring(4, 6);
    const day = param.substring(6, 8);
    const hour = param.substring(9, 11);
    const minute = param.substring(11, 13);
    const seconds = param.substring(13, 15);

    // concatenate substrings above to a date string with format 'yyyy-mm-ddTHH:mm:ss'
    const res = year + '-' + month + '-' + day + 'T' + hour + ':' + minute + ':' + seconds;
